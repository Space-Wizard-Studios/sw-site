...
function getNormDistance(a: React.RefObject<HTMLElement>, b: React.RefObject<HTMLElement>, w: React.RefObject<HTMLElement>) {
...
export function Handle({ isOpen, setOpen, setHasMoved, targetRef, constraintsRef }: Props) {
  ...
  return (
    ...
    >
      <Rocket flames={isMoving ? 1 : 0} className='absolute left-0 right-0 top-0 bottom-0 m-auto w-6 h-6 text-sw-secondary dark:text-sw-primary' style={{ rotate: '45deg' }} />
    </motion.button>
  )
}
