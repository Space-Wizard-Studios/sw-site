...
export function MemberPicture({ name, photoSrc, isOpen }: Props) {
  return (
    <motion.div variants={variants} initial='closed' animate={isOpen ? 'opened' : 'closed'} className='flex absolute w-full h-24 items-center content-center justify-center'>
      <motion.div
        animate={{
          rotate: '360deg',
          transition: {
